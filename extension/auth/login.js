// login.js - Google OAuth authentication logic
...
async function authenticateWithBackend(idToken) {
  try {
    ...
    console.error('Ошибка аутентификации на сервере:', error);
    throw error;
  }
}
  ...
  try {
    ...
    chrome.identity.launchWebAuthFlow(
      ...
      async (redirectUrl) => {
        // Check for errors
        if (chrome.runtime.lastError) {
          console.error('Ошибка OAuth:', chrome.runtime.lastError);
        ...
        try {
          ...
          if (!idToken) {
            throw new Error('ID токен не получен');
          }
          ...
          console.error('Ошибка процесса аутентификации:', error);

 ...
 * Переводит Content Intelligence анализ на русский язык через GPT
 */
export async function POST(
  ...
  try {
    ...
    if (analysisResult.rows.length === 0) {
      client.close();
      return NextResponse.json(
        { error: "No Content Intelligence analysis found. Please generate analysis first." },
        { status: 404 }
      );
    }
...
Return ONLY the translated JSON without any additional text or markdown formatting.`,
    ...
    await client.execute({
      sql: `UPDATE content_intelligence
            SET data_ru = ?
            WHERE channelId = ?
            AND id = (
    ...
    console.error("[ContentIntelligenceTranslate] Error:", error);
    ...
    return NextResponse.json(
      { error: "Failed to translate Content Intelligence analysis" },
      { status: 500 }
    );
  }
}

...
Выдели 5-10 основных тем контента в виде таблицы Markdown:

| Тема | Кол-во видео | Средний просмотр | Тренд |
|------|---|---|---|
| Название темы | 3-5 | 10000-50000 | Растёт/Падает/Стабильна |

Для каждой темы:
- Используй реальные названия из названий видео, а не обобщение
- Указывай количество видео на эту тему
- Средний просмотр
- Тренд популярности (растёт/падает или стабильна)

**ФОРМАТ ТАБЛИЦЫ:** Используй корректный Markdown-формат таблицы БЕЗ псевдографики:
| Col1 | Col2 | Col3 | Col4 |
|------|------|------|------|
| …    | …    | …    | …    |

## 3. ФОРМАТЫ КОНТЕНТА (formats)
Определи 4-8 основных форматов (структур) видео в виде таблицы Markdown:

| Формат | Кол-во видео | Средний просмотр | Особенности |
|--------|---|---|---|
| Обзор/Туториал/Влог | 5-10 | 20000-100000 | Примеры и описание |

Для каждого формата:
- Тип формата (обзор, туториал, влог, интервью, челлендж и т.д.)
- Сколько видео этого формата
- Средний просмотр для формата
- Особенности этого формата в данном канале
...
- Каждый пункт должен быть подтвержден конкретными примерами из названий видео`
        }
      ],
      temperature: 0.6,
    });

 ...
 * Переводит Momentum Insights анализ на русский язык через GPT
 */
export async function POST(
  ...
  try {
    ...
    if (analysisResult.rows.length === 0) {
      client.close();
      return NextResponse.json(
        { error: "No Momentum Insights analysis found. Please generate analysis first." },
        { status: 404 }
      );
    }
...
Return ONLY the translated JSON without any additional text or markdown formatting.`,
    ...
    await client.execute({
      sql: `UPDATE momentum_insights
            SET data_ru = ?
            WHERE channelId = ?
            AND id = (
              SELECT id FROM momentum_insights
              WHERE channelId = ?
              ORDER BY generatedAt DESC
              LIMIT 1
            )`,
    ...
    console.error("[MomentumTranslate] Error:", error);
    ...
    return NextResponse.json(
      { error: "Failed to translate Momentum Insights analysis" },
      { status: 500 }
    );
  }
}

...
## 2025-12-17 - Упрощение платежной системы YooKassa (максимально простая схема)

### Кардинальное упрощение архитектуры платежей

Переделана платежная система по принципу "максимально простая схема без подписок". Единственная точка активации тарифа — webhook от YooKassa. Удалены все сложности: billingCycle, metadata для cycl, несколько точек активации.

#### Главные изменения

1. **Webhook (`/api/payments/yookassa/webhook`) — ЕДИНСТВЕННАЯ ТОЧКА АКТИВАЦИИ**
   - Получает event `payment.succeeded` от YooKassa
   - Проверяет идемпотентность ДО любых действий (SELECT на externalPaymentId)
   - **АКТИВИРУЕТ** тариф: `UPDATE users SET plan = ?, expiresAt = now + 30 дней`
   - Логирует платеж в таблицу `payments`
   - Никакого другого функционала

2. **Confirm endpoint (`/api/payments/yookassa/confirm`) — БЕЗ влияния на тариф**
   - Только ПРОВЕРЯЕТ статус платежа для информации
   - НЕ изменяет тариф пользователя
   - НЕ логирует платеж (уже залогирован в webhook)
   - Комментарий: "Активация тарифа УЖЕ произошла в webhook!"

3. **Удалены из кода**
   - `billingCycle` из metadata полностью
   - `metadata.yearly/monthly` — ВСЕГДА только 30 дней (хардкод)
   - Никаких проверок окружения, fallback, флагов
   - Никакой логики на "будущее"

4. **Срок подписки ВСЕГДА = 30 дней**
   - `expiresAt = now + 30 * 24 * 60 * 60 * 1000` (хардкод в webhook)
   - Один платеж = один апдейт тарифа
   - Downgrade по условию: окончание срока ИЛИ закончились лимиты (уже было, работает в `getUserPaymentInfo`)

5. **Чистка функций**
   - `updateUserPlan()` в `payments.ts` — убрана логика сброса usage (лишняя)
   - Осталось только: UPDATE users по плану и срку

#### Архитектурные принципы (ФИНАЛ, НЕ ОБСУЖДАЕТСЯ)
- ✅ Единственная точка активации = webhook
- ✅ Confirm НЕ используется для активации
- ✅ billingCycle удалена полностью
- ✅ Срок подписки ВСЕГДА 30 дней
- ✅ Один платеж = один UPDATE users
- ✅ Идемпотентность ДО действий
- ✅ Никаких fallback, проверок, флагов
...
#### Изменения на лендинге (/landing)
- Обновлены названия планов: Basic, Professional, Enterprise
- Обновлены подзаголовки планов на русском
- Обновлены описания функций для каждого плана
- Обновлены тексты кнопок:
  - Basic: "Сгенерировать сценарий"
  - Professional: "Создавать сценарии регулярно"
  - Enterprise: "Использовать в работе"
- Обновлены цены на русский рубль:
  - Basic: 990 ₽ в месяц (9900 ₽ в год)
  - Professional: 2 490 ₽ в месяц (24 900 ₽ в год)
  - Enterprise: 5 990 ₽ в месяц (59 900 ₽ в год)
- Добавлен блок информации о Free плане (вариант A):
  - Текстовый блок перед тарифными карточками
  - До 3 AI-сценариев в месяц
  - Генерация по YouTube-ссылке
  - Без обновления аналитики и повторной генерации
  - Подходит для ознакомления перед оплатой
- Обновлены английские тексты на русские:
  - "Pricing Plans" → "Тарифные планы"
  - "Choose your plan" → "Выберите подходящий план"
  - "Monthly/Annually" → "Помесячно/Годовой"

#### Изменения на странице /settings/billing
- Обновлены названия планов для отображения
- Обновлены цены в представлении текущего плана
- Обновлены тексты кнопок для режима billing (на русском)

#### Затронутые файлы
- `src/components/pricing-plans.tsx` - главный компонент тарифов (для billing)
- `src/app/landing/components/pricing-section.tsx` - компонент тарифов на лендинге
- `src/app/(dashboard)/settings/billing/page.tsx` - отображение текущего плана и цен
- `CHANGES.md` - документирование изменений

#### Примечание по логике подписок
- Логика валидации и управления подписками НЕ изменена
- ID планов остаются прежними (basic, professional, enterprise, free)
- Лимиты в базе данных НЕ изменены
- Backend не затронут
- Функциональность покупки и смены планов не изменена

---

## 2025-12-13 - Активация кнопки "Сгенерировать сценарий" для режима одиночного видео

### Обновлена логика активации кнопки на странице /trending

#### Изменения
- Кнопка "Сгенерировать сценарий" теперь становится активной в двух случаях:
  1. Режим "Трендовые видео" - когда выбрано хотя бы одно видео из таблицы
  2. Режим "Одиночное видео" - когда в поле для URL введена ссылка на YouTube-видео
- Обновлена функция `generateScripts()` для поддержки обоих режимов
  - Режим "trending" отправляет массив `selectedVideoIds` (существующее поведение)
  - Режим "youtube" отправляет параметр `youtubeUrl` в формате `sourceMode: "youtube"`
- API уже поддерживает YouTube режим через параметр `sourceMode` и `youtubeUrl`
- Обновлен текст кнопки:
  - В режиме "Трендовые видео": "Сгенерировать сценарий (N)"
  - В режиме "Одиночное видео": "Сгенерировать сценарий"

#### Затронутые файлы
- `src/app/(dashboard)/trending/page.tsx` - обновлена логика активации кнопки и функция генерации

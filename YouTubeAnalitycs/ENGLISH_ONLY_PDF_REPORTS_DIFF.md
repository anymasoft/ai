...
AI-генерируемые отчеты (semantic, skeleton) - **ТОЛЬКО АНГЛИЙСКИЙ**.
Готовые отчеты (script, insights) - **БЕЗ ОГРАНИЧЕНИЙ** (язык как есть).
...
- ✅ Готовые отчеты (script, insights): без валидации, генерируются как есть

...
const transcriptState = {
  ...
  maxFreeLine: -1, // Максимальная строка для Free плана
  userPlan: 'Free', // Текущий план пользователя
...
const realtimeHighlighter = {
  ...
  // Подсветка конкретного элемента (улучшенная версия с учетом ограничений)
  highlight(index) {
    ...
    // Находим элемент по data-index (надежный метод, независимо от положения в DOM)
...
function updateExportDropdownState() {
  ...
  translatedOptions.forEach(option => {
    if (!transcriptState.exportAllowed || !transcriptState.isProcessed) {
      option.classList.add('locked');
      option.setAttribute('data-tooltip', !transcriptState.exportAllowed ? 'Upgrade to Premium' : 'Translation in progress');
    } else {
...
function handleExportFormat(format, type) {
  ...
  let subtitles, filename;

  if (type === 'original') {
    // Используем оригинальные субтитры из transcriptState
    subtitles = transcriptState.subtitles;
    filename = `${videoId}_original.${format}`;
  } else if (type === 'translated') {
    // Для переведенных субтитров проверяем, что не идет обработка
    if (transcriptState.isProcessing) {
      return;
    }

    // Собираем переведённые субтитры из DOM (не из transcriptState!)
    subtitles = collectTranslatedSubtitles();
    filename = `${videoId}_${lang}_translated.${format}`;

    // Дополнительная проверка для переведенных субтитров
    if (!subtitles || subtitles.length === 0) {
      console.error('No translated subtitles found in DOM');
      return;
    }

    // Проверяем, разрешен ли экспорт переведенных субтитров (только для Premium и после завершения перевода)
    if (!transcriptState.exportAllowed || !transcriptState.isProcessed) {
      console.error('Export of translated subtitles is not allowed for current plan or translation not completed');
      return;
    }
  } else {
...
async function handleGetTranscript() {
  ...
  try {
    ...
    // Обновляем план пользователя с учетом новых субтитров
    await updateUserPlan();

    // Отображаем оригинальные субтитры сразу
    displayTranscript(subtitles);
    updateLimitedClass(); // Применяем ограничения сразу после создания элементов
...
// Функция для расчета максимальной строки для Free плана
function calculateMaxFreeLine(totalLines) {
  return Math.max(1, Math.floor(totalLines * 0.3) - 1); // 30% строк, минимум 1
}

// Обновление плана пользователя и ограничений
async function updateUserPlan() {
  try {
    ...
    // Разрешаем экспорт только для Premium плана
    transcriptState.exportAllowed = userPlan === 'Premium';
    console.log(`Экспорт разрешен: ${transcriptState.exportAllowed} (план: ${userPlan})`);

    // Удаляем кнопки апгрейда если план не Free
    if (userPlan !== 'Free') {
      removeUpgradeButtons();
    }

    // Обновляем maxFreeLine если есть субтитры
    if (transcriptState.subtitles) {
      if (userPlan === 'Free') {
        transcriptState.maxFreeLine = calculateMaxFreeLine(transcriptState.subtitles.length);
      } else {
        transcriptState.maxFreeLine = transcriptState.subtitles.length; // Нет ограничений
      }
      
      // Применяем классы ограничения
      updateLimitedClass();
    }
    ...
    console.log(`План пользователя обновлен: ${userPlan}, maxFreeLine: ${transcriptState.maxFreeLine}, exportAllowed: ${transcriptState.exportAllowed}`);
  } catch (error) {
...
async function translateSubtitles(videoId, subtitles) {
  ...
  // Рассчитываем максимальную строку для Free плана
  if (userPlan === 'Free') {
    transcriptState.maxFreeLine = calculateMaxFreeLine(subtitles.length);
    console.log(`Free план: показываем ${transcriptState.maxFreeLine + 1} из ${subtitles.length} строк`);
  } else {
    transcriptState.maxFreeLine = subtitles.length; // Нет ограничений
  }

  console.log(`Начинаем перевод на ${selectedLang}... План: ${userPlan}`);
  ...
  async function translateLineWithRetry(i, subtitle, retryCount = 0) {
    try {
      // Проверяем ограничение Free плана
      if (userPlan === 'Free' && i > transcriptState.maxFreeLine) {
        console.log(`Free план: пропускаем строку ${i} (превышен лимит ${transcriptState.maxFreeLine})`);
        return null;
      }
      ...
      // Проверяем серверное ограничение (для совместимости со старой версией)
      if (data.stop === true) {
        console.log(`Серверное ограничение: остановка на строке ${i}`);
        transcriptState.maxFreeLine = i - 1;
        return null;
      }
  ...
  try {
    ...
    for (let i = 0; i < subtitles.length; i += BATCH_SIZE) {
      ...
      for (let j = i; j < Math.min(i + BATCH_SIZE, subtitles.length); j++) {
        // Проверяем, не превышен ли лимит для Free плана
        if (userPlan === 'Free' && j > transcriptState.maxFreeLine) {
          console.log(`Free план: остановка перевода на строке ${j}`);
          break;
        }
      ...
      // Если это Free план и мы достигли лимита, добавляем кнопки апгрейда
      if (userPlan === 'Free' && lastTranslatedIndex >= transcriptState.maxFreeLine) {
        console.log(`Free план: достигнут лимит, добавляем кнопки апгрейда после строки ${lastTranslatedIndex}`);
        insertUpgradeButtons(lastTranslatedIndex);
        
        // Применяем классы ограничения для скрытия строк после лимита
        updateLimitedClass();
        break;
      }
    }

    console.log(`Перевод завершен: ${lastTranslatedIndex + 1} из ${subtitles.length} строк на ${selectedLang}. План: ${userPlan}`);
    
    // Для Free плана: если кнопки апгрейда еще не добавлены (например, все строки уместились в лимит),
    // добавляем их после последней переведенной строки
    if (userPlan === 'Free' && !document.querySelector('.yt-upgrade-marker')) {
      console.log(`Free план: добавляем кнопки апгрейда после полного перевода`);
      const finalIndex = Math.min(lastTranslatedIndex, transcriptState.maxFreeLine);
      insertUpgradeButtons(finalIndex);
    }
    
    // Финальное применение ограничений - гарантируем, что все строки после лимита имеют правильную прозрачность
    updateLimitedClass();
...
function resetState() {
  ...
  transcriptState.maxFreeLine = -1;
  transcriptState.userPlan = 'Free';

  // Удаляем кнопки апгрейда
  removeUpgradeButtons();

  // Сбрасываем классы ограничения
  document.querySelectorAll('.yt-transcript-item.limited').forEach(item => {

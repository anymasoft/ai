...
try:
    ...
    print(f"Warning: google.generativeai not available. Gemini will not work.")
...
class Gemini(Bot):
    def __init__(self, key_path, patience=3, model="gemini-1.5-flash-latest") -> None:
        if not GEMINI_AVAILABLE:
            raise RuntimeError("google.generativeai not available. Please install with: pip install google-generativeai")
        super().__init__(key_path, patience)

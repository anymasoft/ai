/**
 * Beem MVP Telegram Bot (встроен в Astro)
...
const TEMP_DIR = path.join(process.cwd(), 'tmp', 'telegram-bot');

/**
 * Инициализировать временную директорию
 */
function ensureTempDir(): void {
  if (!fs.existsSync(TEMP_DIR)) {
    fs.mkdirSync(TEMP_DIR, { recursive: true });
    console.log(`[TELEGRAM-BOT] Temp directory created: ${TEMP_DIR}`);
  }
}
...
function logEvent(
  ...
  const msg = `[${timestamp}] [TELEGRAM-BOT] [${eventType}] user=${userId}`;
...
export async function createAndStartBot(): Promise<Bot | null> {
  if (!TOKEN) {
    console.warn('[TELEGRAM-BOT] ⚠️  TELEGRAM_BOT_TOKEN не установлен. Бот не будет запущен.');
    return null;
  }
...
Начнём? 👇`;
  ...
  bot.on('message:photo', async (ctx: Context) => {
    ...
    try {
      ...
      console.error(`[TELEGRAM-BOT] Photo download error: ${msg}`);
...
Всё верно?`;
  ...
  bot.callbackQuery('confirm_generate', async (ctx: Context) => {
    ...
    try {
      ...
      const pollStatus = async (): Promise<void> => {
        while (attempts < maxAttempts) {
          try {
            ...
            const msg = error instanceof Error ? error.message : 'Unknown error';
            console.error(`[TELEGRAM-BOT] Status check error: ${msg}`);
      ...
      const msg = error instanceof Error ? error.message : 'Unknown error';
      console.error(`[TELEGRAM-BOT] Generation error: ${msg}`);
  ...
  console.log('[TELEGRAM-BOT] Bot instance created');
  return bot;
}
...
export async function startBot(bot: Bot): Promise<void> {
  try {
    ...
    console.log(`[TELEGRAM-BOT] Backend URL: ${BASE_URL}`);

    // Проверяем backend
    const isHealthy = await checkBackendHealth();
    if (!isHealthy) {
      console.warn(
        `[TELEGRAM-BOT] ⚠️  Backend may not be available at ${BASE_URL}`
      );
      console.warn('[TELEGRAM-BOT]    Make sure Astro is running properly');
    } else {
      console.log('[TELEGRAM-BOT] ✅ Backend is available');
    }

    // Запускаем бота в polling режиме (без блокирования)
    const abortController = new AbortController();
    bot
      .start({
        onStart: (botInfo) => {
          console.log(`[TELEGRAM-BOT] ✅ Bot logged in as @${botInfo.username}`);
          console.log('[TELEGRAM-BOT] ✅ Telegram bot is running!');
        },
        signal: abortController.signal,
      })
      .catch((err) => {
        console.error('[TELEGRAM-BOT] Bot error:', err);
      });
  } catch (error) {
    const msg = error instanceof Error ? error.message : 'Unknown error';
    console.error(`[TELEGRAM-BOT] Failed to start bot: ${msg}`);
  }
}

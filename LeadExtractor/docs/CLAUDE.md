...
## 🎯 Data Extraction Tool (v2.3 - Current)

### Трансформация: Lead Management → Pure Data Tool

**v3.0 (Removed):**
- ❌ Lead Score badge (50x50 px)
- ❌ Status tags (HOT/WARM/COLD)
- ❌ Analytics-focused UI
- ❌ Gradient badges
- ❌ Complex color coding

**v2.3 (Current):**
- ✅ Clean data extraction table
- ✅ Only real data (no analytics)
- ✅ Deduplicated sources (Set)
- ✅ Copyable sources (click to copy)
- ✅ 5 essential columns: Website, Emails, Phones, Sources, Action
- ✅ Simple, focused UI
...
```
┌────────────────────────────────────────────────────────┐
│ Website  │ Emails     │ Phones     │ Sources  │ Action  │
├──────────┼────────────┼────────────┼──────────┼─────────┤
│ 1cca.ru  │ info@...   │ +7 (495).. │ / [Copy] │ ✉ Email │
│ (link)   │ contact@.. │            │ /about   │         │
│          │            │            │ /contact │         │
└──────────┴────────────┴────────────┴──────────┴─────────┘
```
...
- Font-weight: 600
- Displays full URL with title

#### 2. Emails List
- All found emails in one cell
- mailto: links (click to open email client)
- Organized in flex column

#### 3. Phones List
- All found phones in one cell
- tel: links (click to call)
- Whitespace removed for tel protocol

#### 4. Sources Column (Deduplicated)
- **Unique values only** (using Set)
- **Copyable on click** with visual feedback
...
#### 5. Action Column
- "✉️ Email" button
- Opens email client with first contact
- Only shown if emails exist

### CSS Classes (Simplified)

```css
.data-table { /* Main table styling */ }
.data-row { /* Row with hover effect */ }
.website-cell { /* Website column */ }
.contacts-cell { /* Emails/Phones columns */ }
.contact-link { /* Individual email/phone */ }
.sources-cell { /* Sources column */ }
.source-item { /* Individual source - clickable */ }
.source-item.copied { /* Highlight after copy */ }
.copy-indicator { /* ✓ Copied text */ }
...
**Example:**
```csv
Website,Emails (Count),Phones (Count),Sources
1cca.ru,2,1,/;/about;/contacts
company.io,1,1,/;/team;/contact
example.com,0,1,/;/inquiry
```

**No Score or Status** - just pure data extraction data.

### Functions (Simplified)

#### formatUrl(urlString)
```javascript
// Truncates URL to 35 chars
// Adds "..." if longer
// Returns pathname or original string on error
```

#### handleSourceClick(source, idx)
```javascript
// Copies source to clipboard
...
**Displayed only:**
- Sites with email OR phone
- Empty sites are filtered out
- Shows "No contacts found" message if empty

### User Experience

✅ **Clean, minimal UI** - focus on data
✅ **No distractions** - no scores, no status
✅ **Copyable sources** - click to copy to clipboard
✅ **Deduplicated data** - no duplicate sources
✅ **Interactive contacts** - mailto and tel links
✅ **Email button** - quick outreach
...
❌ Status tags (.status-tag, .status-hot/warm/cold)
❌ Score/Status columns
❌ Gradient badges
❌ Color-coded lead quality
❌ "Lead Management UI" concept
❌ All analytics-related logic

### Result

**Pure data extraction tool that:**
- Shows only real data
- Has no duplicates
- Is maximally simple and readable
- Works as extraction tool, not analytics
- Exports clean CSV for CRM/follow-up
...
### Backend (v2.0)
- 3-layer architecture (FETCH → EXTRACTION → TRAVERSAL)
- Stable BFS traversal (5-10 pages)
- Contacts with sources
- Error handling at each layer

### Frontend (v2.3)
- Clean data extraction table
- 5 columns: Website, Emails, Phones, Sources, Action
- Clickable/copyable data
- No analytics or scoring
- Simple, focused UI

### Both Together
- Production-ready contact extraction system
- No over-engineering
- Pure data focus

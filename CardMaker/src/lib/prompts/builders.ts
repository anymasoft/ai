 ...
 * 6. (ОПЦИОНАЛЬНО) Описания похожих товаров, если есть
...
- Правильная пунктуация и орфография`
  ...
  // Проверяем наличие валидных описаний похожих товаров (ОПЦИОНАЛЬНО)
...
${seoKeywords.filter((k) => k && k.trim().length > 0).join(', ')}`)
  ...
  // Добавляем описания похожих товаров ТОЛЬКО если есть хотя бы одно описание
  if (hasCompetitors) {
    userPromptParts.push(`\nДля вдохновения (описания похожих товаров):

  ...
  async function saveTariff() {
    ...
    try {
      ...
      if (isNaN(price_rub) || isNaN(credits)) {
        toast.error("Цена и количество операций должны быть числами")
        return
      }

      if (price_rub < 0 || credits < 0) {
        toast.error("Цена и количество операций не могут быть отрицательными")
        return
      }
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">Редактируйте цены и количество операций</p>
        </div>
        <Button
          variant="outline"
          size="icon"
